...
class FFXIVItem:
    ...
    def get_garland_info(self) -> FFXIVItem:
        ...
        if item is None:
            self.logger.warning(
                "Failed to find any information on Item ID: %s | Item Name: %s",
                self.item_id,
                self.en_name,
            )
            return self
        ...
        for key, value in item.items():
            ...
            setattr(
                self,
                "ffxiv_wiki",
                f"https://ffxiv.consolegameswiki.com/wiki/{self.name.replace(' ', '_')}",
            )
        return self
    ...
    def get_fish_catching(self) -> Any:
        ...
        if len(spots) > 0:
            ...
            for entry in spots:
                data: GarlandToolsAPI_FishingLocationsTyped | None = self._garland_api.fishing(
                    spot_id=entry.get("spot", 0)
                )
                if data is None:
                    continue
    ...
    def get_patch_icon(self) -> discord.File:
        ...
        """
        return discord.File(
            fp=FFXIVResource.resource_path.joinpath(f"{self.patch.name}-icon.png"),
            filename=f"{self.patch.name}.png",
        )
...
class FFXIVUser:
    ...
    def __init__(self, data: FFXIVUserDBTyped, db_pool: asqlite.Pool) -> None:
    ...
    async def get_watch_list_items(self) -> list[FFXIVItemWatchList]:
        ...
        async with self.pool.acquire() as conn:
            res: list[FFXIVWatchListDBTyped] = await conn.fetchall(
    ...
    async def update_watch_list_item(self, item: FFXIVItemWatchList) -> FFXIVItemWatchList:
        async with self.pool.acquire() as conn:
            res: FFXIVWatchListDBTyped = await conn.fetchone(  # type: ignore
...
class FFXIVResource:
    ...
    aethernet_icon: ClassVar[discord].File = discord.File(
        fp=resource_path.joinpath("aethernet-icon.png"), filename="aethernet-icon.png"
    )
    item_banner: ClassVar[discord].File = discord.File(
        fp=resource_path.joinpath("ffxiv-trail-banner.png"), filename="ffxiv-banner.png"
    )
    universalis_icon: ClassVar[discord].File = discord.File(
        fp=resource_path.joinpath("universalis-icon.png"), filename="uni-icon.png"
    )
    garlandtools_icon: ClassVar[discord].File = discord.File(
        fp=resource_path.joinpath("garlandtools-icon.png"), filename="gt-icon.png"
    )

    @classmethod
    def get_banner(cls) -> discord.File:
        return discord.File(
            fp=cls.resource_path.joinpath("ffxiv-trail-banner.png"), filename="ffxiv-banner.png"
        )
    ...
    def get_aethernet_icon(cls) -> discord.File:
        return discord.File(
            fp=cls.resource_path.joinpath("aethernet-icon.png"), filename="aethernet-icon.png"
        )
    ...
    async def build_item_list(self) -> None:
        ...
        if data.status != 200:
            self.logger.error(
                "We encountered an error retrieving our items.json data. | URL: %s | Status: %s",
                self.item_url,
                data.status,
            )
    ...
    def item_name_to_ids(
        self, item_name: str, language: str = "en", match: int = 80, limit_results: int = -1
    ) -> list[FFXIVItem]:
        """
        ...
        for item in self.items:
            ...
            if item_name.lower() == getattr(item, f"{language}_name", "UNK").lower():
                ...
                ratio: int = fuzz.partial_ratio(
                    s1=getattr(item, f"{language}_name", "UNK").lower(), s2=item_name.lower()
                )
        ...
        self.logger.info(
            "Universalis: Converting Item Name: %s to IDs: %s", item_name, matches[:limit_results]
        )
...
class GarlandAPIWrapper(GarlandTools):
    ...
    def icon(
        ...
        self.logger.error(
            "We encountered an error looking up this Icon ID: %s Type: %s for GarlandTools. | Status Code: %s",
            icon_id,
            icon_type,
            res.status_code,
        )
